...
export const AGENT_TOOLS: MMTool[] = [
  ...
  },
  {
    ...
    function: {
      name: "read_book_toc",
      description:
        "读取某馆藏书的完整目录（全书概要 + 各章标题与概要）。【何时调用】答疑 / 解读该书前先调用。**概览类问题（这本书讲什么 / 核心观点）凭目录里的概要即可直接作答、不必再细读章节**；仅需具体章节 / 情节 / 原文细节时才继续 read_chapter。book_id 取自〔图书馆书单〕的 [id]。",
      parameters: {
        type: "object",
  ...
  },
  {
    ...
    function: {
      name: "read_chapter",
      description:
        "读取某一章的完整原文。【何时调用】仅当读者问到具体章节 / 情节 / 人物细节、或要引用原文时调用，据原文讲准、别凭记忆编。一次答疑通常细读 1~2 章即可，**别整本逐章读**（概览问题用 read_book_toc 的概要就够）。chapter_no：0 = 前言，1 起为正文。",
      parameters: {
        type: "object",
        properties: {
          book_id: { type: "string" },
  ...
  },
  {
    ...
    function: {
      name: "web_lookup",
      description:
        "联网获取互联网实时公开信息。【必须调用】① 读者要你联网 / 查最新；② 实时时效内容（天气、新闻热搜、股价 / 汇率 / 油价 / 金价、比分、票房、最新版本 / 型号 / 价格、谁获奖当选）；③ 上一轮在聊天气 / 新闻等实时话题时，读者的省略式追问（「那明天呢」「北京呢」）；④ 读书之外需事实查证、超出你知识截止的提问。**先查到再答，绝不凭印象编实时数据。**【勿用】馆藏书内容 / 读书方法 / 荐书 / 常识概念，直接答不联网。【query】精炼中文关键词；问「最新」带上当前年月（如「{当前年月} …」），以带日期的新结果为准。结果来源卡由系统展示。",
      parameters: {
        type: "object",
