// 验证「卡片交错渲染」（T4 单一会话版）：推荐/引用卡按工具调用在对话流中的真实位置渲染
//   ① 荐书：气泡内出现推荐卡，且卡片前后都有正文段（交错成立）
//   ② 正文不泄漏占位标记 [[recs/[[cites
//   ③ 云端 main 落库的 content 携带标记（历史回显可按位置还原）
//   ④ 答疑（同一会话流接着问）：引用卡出现且无标记泄漏
//   ⑤ demo 是共享体验账号：开测前备份 main 原样，测后恢复
...
const { data: backup } = await admin.from("chat_sessions").select("*").eq("user_id", demo.id).eq("id", "main").maybeSingle();
...
async function restore() {
  ...
  else await admin.from("chat_sessions").delete().eq("user_id", demo.id).eq("id", "main");
...
try {
  ...
  // 单一会话验收：页面无任何会话管理入口
  const noMgmt = await page.evaluate(() => {
    const t = document.body.textContent || "";
    return !document.querySelector('a[href*="/chat/history"]') && !document.querySelector('[aria-label="开启新对话"]') && !t.includes("历史会话");
  });
  ok(noMgmt, "界面无历史会话/新建会话入口（T4）");
