...
const DOCS: Record<Doc, { title: string; sections: { h?: string; p: string }[] }> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { h: "三、信息的存储与保护", p: "我们采取合理的技术与管理措施保护你的信息安全。你的账号数据（收藏、笔记、书评、阅读进度与对话记录）加密存储于云端数据库，并通过行级安全策略确保仅你本人可访问。" },
      { h: "四、你的权利", p: "你有权查看、更正或删除你的个人信息。退出登录仅清空本机缓存，云端数据保留；在设置中注销账号可永久删除你的全部云端数据。" },
